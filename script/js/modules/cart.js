...
const Cart = {
  ...
  mapData() {
    console.log("hassn i fafds fadsf 3234224");
  },
  getData() {
    const _ = this;
  ...
  setActiveAdditions($item, payload) {
    ...
    _.activeItem.totalPrice =
      _.activeItem.sizes.find((o) => o.active === true).price +
      _.activeItemAdditionsTotal;
  ...
  setActiveSize($item, payload) {
    ...
    newSize.active = true;
    _.activeItem.totalPrice =
      _.activeItem.totalPrice - oldSize.price + newSize.price;
  ...
  setActiveItem(id) {
    ...
    _.activeCategory = _.categories.find(
      (o) => o.id === _.activeItem.categoryId
    );
  },
  Panel: {
    DOM: {
        ...
        const $item = $(`
          <tr>
              <td class="title">
                  ...
                  <span class="caption text-muted">${size.name} (${
          size.value
        })</span>
              </td>
  ...
  Modal: {
    ...
    build() {
      ...
      try {
          ...
          Cart.activeItem.sizes.forEach((item) => {
            ...
            <div class="form-group">
              ...
              item.active ? "checked" : ""
                    ...
                    <span class="custom-control-description">${item.name} - ${
              item.value
            } <span>(${currency}${item.price.toFixed(2)})</span></span>
                </label>
            </div>
          `);
        ...
        // Additions
        if (
          Cart.activeCategory.additions &&
          Cart.activeCategory.additions.length > 1
        ) {
          ...
          Cart.activeCategory.additions.forEach((item) => {
            ...
            <div class="col-sm-6">
              ...
              Cart.activeItem.additions &&
              Cart.activeItem.additions.findIndex((o) => o.id === item.id) !==
                -1
                ? "checked"
                : ""
            }>
                        ...
                        <span class="custom-control-description">${
                          item.name
                        } <span>(${currency}${item.price.toFixed(
              2
            )})</span></span>
                    </label>
                </div>
        ...
        console.error(
          "[CART_MODAL] Please check a JSON data source and data-id attribute on the button."
        );
      }
    },
    showProductModal(id) {
